...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'bounce_rate_7d', (
      SELECT ROUND(100.0 * COUNT(*) FILTER (WHERE cnt = 1) / NULLIF(COUNT(*), 0), 1)
      FROM (
    ...
    'hourly_distribution', (
      SELECT COALESCE(json_agg(row_to_json(t) ORDER BY t.hour), '[]'::json) FROM (
        ...
        FROM site_visits WHERE created_at >= now() - interval '24 hours'
        GROUP BY 1
      ) t
    ),
    ...
    'bot_by_agent', (
      SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json) FROM (
        ...
        FROM site_visits
        WHERE is_bot = true AND created_at >= now() - interval '7 days' AND user_agent IS NOT NULL
        GROUP BY user_agent
    ...
    'suspicious_ips', (
      SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json) FROM (
        SELECT ip_hash, count(*) as hits, max(created_at) as last_seen,
               ...
               (array_agg(city_name) FILTER (WHERE city_name IS NOT NULL))[1] as city_name,
               (array_agg(user_agent) FILTER (WHERE user_agent IS NOT NULL))[1] as user_agent_sample
        FROM site_visits
