-- Enhance get_access_analytics with richer user and traffic data:
-- 1. Authenticated vs anonymous visits
-- 2. Bounce rate and avg session depth
-- 3. Hourly traffic distribution (last 24h)
-- 4. Top authenticated users by visit count
-- 5. UTM campaigns breakdown
-- 6. Suspicious IPs now include country/city and user_agent sample
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'anonymous_visits_24h', (SELECT count(*) FROM site_visits WHERE user_id IS NULL AND is_bot = false AND created_at >= now() - interval '24 hours'),

    -- Session quality metrics (7d, excluding bots)
    'bounce_rate_7d', (
      SELECT ROUND(
        100.0 * count(*) FILTER (WHERE pv = 1) / NULLIF(count(*), 0), 1
      )
      FROM (
    ...
    -- Hourly distribution for the last 24h (real users only)
    'hourly_distribution', (
    ...
    'top_utm_campaigns', (
      SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json) FROM (
        SELECT
          utm_campaign,
          COALESCE(utm_source, 'desconhecido') as utm_source,
          COALESCE(utm_medium, 'desconhecido') as utm_medium,
          count(*) as visits,
          count(DISTINCT session_id) as unique_sessions
        FROM site_visits
        WHERE created_at >= now() - interval '30 days' AND utm_campaign IS NOT NULL
        GROUP BY utm_campaign, utm_source, utm_medium ORDER BY visits DESC LIMIT 10
      ) t
    ),
    'top_countries', (
      SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json) FROM (
    ...
    -- Top authenticated users (user_id + visit counts, no PII)
    'top_authenticated_users', (
      SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json) FROM (
        SELECT
          sv.user_id,
          COALESCE(u.full_name, 'Usuário #' || left(sv.user_id::text, 8)) as display_name,
          COALESCE(u.role, 'desconhecido') as role,
          count(*) as visits,
        ...
        WHERE sv.user_id IS NOT NULL AND sv.created_at >= now() - interval '7 days'
        GROUP BY sv.user_id, u.full_name, u.role
    ...
    'suspicious_ips', (
      SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json) FROM (
        SELECT
          ...
          COALESCE(max(country_code), 'Unknown') as country_code,
          COALESCE(max(city_name), '') as city_name,
          (array_agg(user_agent ORDER BY created_at DESC) FILTER (WHERE user_agent IS NOT NULL))[1] as user_agent_sample
        FROM site_visits
