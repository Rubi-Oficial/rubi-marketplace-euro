...
export default function LocalSeoPage() {
  ...
  const breadcrumbs = [
    ...
    { name: MARKET_LABEL[data.city.market], url: buildAbsoluteUrl(`/${data.city.market}`) },
    { name: data.city.cityName, url: buildAbsoluteUrl(data.city.basePath) },
    ...(data.page.slug ? [{ name: data.page.h1, url: buildAbsoluteUrl(path) }] : []),
  ];
  ...
  usePageMeta({
    ...
    hreflang: buildHreflangUrls(path),
  });
