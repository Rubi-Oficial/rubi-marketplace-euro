...
export async function fetchEligibleProfiles(filters?: {
  ...
  return filteredProfileIds.map((id) => {
    ...
    return {
      ...
      highlight_tier: (p as any).highlight_tier ?? "standard",
      highlight_expires_at: (p as any).highlight_expires_at ?? null,
      image_urls: imageMap[p.id!] || [],
      bio: p.bio ?? null, has_whatsapp: (p as any).has_whatsapp ?? false,
    };
  });
}
