...
### 1) Busca e filtros no mobile (Alta)
...
### 2) Consistência de espaçamentos e densidade (Alta)
...
### 3) Acessibilidade de contraste e foco (Alta)
...
### 4) Navegação por categorias (Média)
...
### 5) Estados vazios e carregamento (Média)
...
1. Melhorar sticky actions de filtro no mobile.
2. Padronizar espaçamentos principais da home e navbar.
3. Fazer revisão rápida de contraste e foco (teclado).
4. Melhorar navegação de categorias no mobile.
